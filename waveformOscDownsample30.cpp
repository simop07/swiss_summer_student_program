...
constexpr int nMaxAnalysedRows{1000};  // Maximum rows INCLUDED (1001 4LayersA)
...
void waveformAnalysis() {
  ...
  std::ifstream infile("./data/incT.txt");
  std::string line;
  ...
  while (std::getline(infile, line)) {
    ...
    std::vector<double> samplesDownsampled;
    double timestamp = 0.;
    int column = 1;
    ...
    auto it = std::find_if(
        samplesDownsampled.begin(), samplesDownsampled.end(),
        [&](double val) { return ((val - maxVoltage) < tolerance); });
    if (it != samplesDownsampled.end()) {
      continue;
    }

    // Creating WaveformAnalysis object
    WaveformAnalysisNeg wf(samplesDownsampled, timestamp, samplePeriod);
    ...
    std::for_each(samplesDownsampled.begin(),
                  samplesDownsampled.begin() + nBaselineSamples,
